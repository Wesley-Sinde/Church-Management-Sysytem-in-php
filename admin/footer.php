<div class="setfooter">
	<footer>
		<p align="center">
		<h3 align="center"> &copy; Deliverance Church Donholm System Manager <?php
																				$date = new DateTime();
																				echo $date->format(' Y');
																				?> </h3>
		</p>
		<h3>
			<p align="center">
			<h3 align="center"><strong> Designed by: Lynn Obadha</a></h3></strong><b class="pull-right" align='right' style="text-align: right; color:green; font-size:smaller;" id="CopyRights"></b>
			</p>
			<!-- 		 
		  		  <h3 align="center"> Follow Us :</h3> <p align="center">
<a href="#" ><img src="images/facebook.png" id=social /></a>
<a href="#"><img src="images/twitter.png" id=social /></a>
<a href="#"><img src="images/linkedin.png"id=social  /></a>
<a href="#"><img src="images/youtube.png" id=social /></a>
<a href="#"><img src="images/gmail.png" id=social /></a></p> -->

</div>
<footer>
	</div>